...
#ifdef _SLRING3//_WINDOWS //for ring3
...
void SLDecoder_SetData(SLDECODER_CTX *pDecoderCtx,UCHAR *val,int len){
	pDecoderCtx->_ip = 0;
	pDecoderCtx->_ip = 0;

	pDecoderCtx->_data = val;
	//	pDecoderCtx->_ip = len;

	/*
	pDecoderCtx->_data = (UCHAR*)malloc(len + 1);
	if(pDecoderCtx->_data){
		memcpy(pDecoderCtx->_data,val,len);
		pDecoderCtx->_ip = len;
	}
	*/
}

long SLDecoder_int32(SLDECODER_CTX *pDecoderCtx)
{
	long val;
#ifndef RtlCopyMemory
	memcpy(&val,pDecoderCtx->_data+pDecoderCtx->_ip,sizeof(long));
#else
	RtlCopyMemory(&val,pDecoderCtx->_data+pDecoderCtx->_ip,sizeof(long));
#endif
	pDecoderCtx->_ip += 4;
	return val;
}

void SLDecoder_bytes(SLDECODER_CTX *pDecoderCtx,BYTE *val,int len)
{
#ifndef RtlCopyMemory
	memcpy(val,pDecoderCtx->_data+pDecoderCtx->_ip,len);
#else
	RtlCopyMemory(val,pDecoderCtx->_data+pDecoderCtx->_ip,len);
#endif
	pDecoderCtx->_ip += len;
}


ULONG SL_ImportMpi(SLDECODER_CTX *pSLDecoder, mpi *pRsaMpi)
{
	ULONG dwErr;
	UCHAR buff[1024];
	int i;
    ...
    dwErr = STATUS_UNSUCCESSFUL;

	do
	{
		i = SLDecoder_int32(pSLDecoder);
        SLDecoder_bytes(pSLDecoder,buff,i);
...
ULONG  SLRsa_ImportPrivKey(rsa_context *pRsaCtx,UCHAR *pKey,ULONG dwKeyLen)
{
	SLDECODER_CTX oDecoder;
	ULONG dwErr;
	...
	do
	{
		...
		SLDecoder_SetData(&oDecoder,pKey,dwKeyLen);

		//OutputDebugString("003");
		pRsaCtx->ver = SLDecoder_int32(&oDecoder);
		pRsaCtx->len = SLDecoder_int32(&oDecoder);
		//OutputDebugString(pRsaCtx->ver);
		//OutputDebugString(pRsaCtx->len);

		dwErr = SL_ImportMpi(&oDecoder,&pRsaCtx->N);      /*!<  public modulus    */
		//OutputDebugString(dwErr);
		if(dwErr != 0)
			break;
		dwErr = SL_ImportMpi(&oDecoder,&pRsaCtx->E);      /*!<  public exponent   */
		//OutputDebugString(dwErr);
		if(dwErr != 0)
			break;
		dwErr = SL_ImportMpi(&oDecoder,&pRsaCtx->D);      /*!<  private exponent  */
		//OutputDebugString(dwErr);
		if(dwErr != 0)
			break;

		dwErr = SL_ImportMpi(&oDecoder,&pRsaCtx->P);      /*!<  1st prime factor  */
		//OutputDebugString(dwErr);
		if(dwErr != 0)
			break;
		dwErr = SL_ImportMpi(&oDecoder,&pRsaCtx->Q);      /*!<  2nd prime factor  */
		//OutputDebugString(dwErr);
		if(dwErr != 0)
			break;
		dwErr = SL_ImportMpi(&oDecoder,&pRsaCtx->DP);     /*!<  D mod (P - 1)     */
		//OutputDebugString(dwErr);
		if(dwErr != 0)
			break;
		dwErr = SL_ImportMpi(&oDecoder,&pRsaCtx->DQ);     /*!<  D mod (Q - 1)     */
		//OutputDebugString(dwErr);
		if(dwErr != 0)
			break;
		dwErr = SL_ImportMpi(&oDecoder,&pRsaCtx->QP);     /*!<  inverse of Q % P  */
		//OutputDebugString(dwErr);
		if(dwErr != 0)
			break;

		if( rsa_check_pubkey(  pRsaCtx ) != 0 ||
			rsa_check_privkey( pRsaCtx ) != 0 )
		{
			dwErr = STATUS_UNSUCCESSFUL;
			break;
		}
...
ULONG SLRsa_ImportPubKey(rsa_context *pRsaCtx,UCHAR *pKey,ULONG dwKeyLen)
{
	SLDECODER_CTX oDecoder;
	ULONG dwErr;
